...
mongoose.connect(
  ...
  {
    //mongoose.connect("mongodb://0.0.0.0:27017/jpludo", {
  },
  (err) => {
    if (!err) {
  ...
  ws.on('message', async function (message) {
    ...
    switch (event) {
      case 'gameCreated': {
        ...
        // const status = await Battle.find({
        //   $or: [
        //     { Status: 'new' },
        //     { Status: 'requested' },
        //     { Status: 'running' },
      ...
      case 'game_seen': {
        const openBattle = await Game.find({
          ...
          .populate('Winner');
        const runningBattle = await Battle.find({
          $or: [
            { Status: 'running' },
            { Status: 'pending' },
            { Status: 'conflict' },
      ...
      case 'gameStarted': {
        const openBattle = await Battle.find({Battle_id: data})
          .populate('Created_by', 'Name Phone avatar _id')
          .populate('Accepetd_By', 'Name Phone avatar _id')
          .populate('Winner', 'Name Phone avatar _id');
        // const runningBattle = await Game.find({
        ...
        //   .populate('Accepetd_By', 'Name Phone avatar _id')
        //   .populate('Winner', 'Name Phone avatar _id');
        const battle = {battle_id: data, openBattle: openBattle };
        
        broadcastEvent('gameStart', battle);
